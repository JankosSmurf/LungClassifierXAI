...
class ClassifyNet(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Flatten(),
            nn.Linear(in_features=32, out_features=17),
            nn.Softmax(dim=1)    # wyłączyć do treningu i wizualizacji
        )
...
class ClassifyNet16(nn.Module):
    def __init__(self):
        super(ClassifyNet16, self).__init__()
        self.net = nn.Sequential(
            nn.Unflatten(1, (1,64)),
            nn.Conv2d(1, 2, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(2),
