...
class NewClassifyNet6(nn.Module):
    def __init__(self):
        super(NewClassifyNet6, self).__init__()
...
class NewClassifyNet8(nn.Module):
    def __init__(self):
        super(NewClassifyNet8, self).__init__()
        self.net = nn.Sequential(
            nn.Unflatten(1, (1,64)),
            nn.Conv2d(1, 2, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(2),
    ...
    def forward(self, x):
        arr = self.net(x)  # Zakładamy shape: [batch_size, 8]
        ### Wyłączyć poniższe do treningu
        part1 = F.softmax(arr[:, :4], dim=1)
        part2 = F.softmax(arr[:, 4:], dim=1)
